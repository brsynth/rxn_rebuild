...
def rebuild_rxn(
    ...
    if cache is None:
        ...
        #     # logger=logger
        # )
        cache = rrCache(cspace=cspace, interactive=False, logger=logger)
    ...
    try:
        ...
        logger.error(
            "      + The reaction rule is not known in the cache. Are you sure you provided the right data-type, e.g. mnx3.1, mnx4.4...?"
        )
        return {}
...
def complete_transfo(
    ...
    if legacy:
        logger.debug("Entering in legacy rules mode")

        ## TEMPLATE REACTION
        if rxn_rule["rel_direction"] == -1:
            _tmpl_rxn = {"right": tmpl_rxn["left"], "left": tmpl_rxn["right"]}
        else:
            _tmpl_rxn = {"right": tmpl_rxn["right"], "left": tmpl_rxn["left"]}
        ...
        logger.debug("Entering in new rules mode")
        missing_compounds = {
    ...
    compl_transfo = build_final_transfo(
        trans_input=trans_input, missing_compounds=missing_compounds, logger=logger
    )
    ...
    for side in Reaction.get_SIDES():
        ...
        _side = (
            ...
            if rxn_rule.get("rel_direction") == -1
            else side
        )
...
def check_compounds_number(
    ...
    logger.debug(
        f"Checking number of compounds between {rxn_name_1} and {rxn_name_2}..."
    )
...
def build_final_transfo(
    trans_input: Dict, missing_compounds: Dict, logger: Logger = getLogger(__name__)
) -> Dict:
...
def build_trans_input(transfo: str, logger: Logger = getLogger(__file__)) -> Dict:
    """
    ...
    for side in Reaction.get_SIDES():
        for cmpd in trans[side].split(trans_input["sep_cmpd"]):
            ...
            if len(_list) > 1:
                ...
                _coeff = 1.0
                _cmpd = _list[0]
            if _cmpd not in trans_input[side]:
...
def detect_missing_compounds(
    ...
    for side in Reaction.get_SIDES():
        ...
        diff_cmpds = dict((Counter(tmpl_rxn[side]) - Counter(rxn_rule[side])))
        ...
        for cmp_id, cmp_sto in diff_cmpds.items():
            if cmp_id in cmpds_to_ignore:
                logger.warning(
                    f"      + Ignoring compound {cmp_id} ({cmp_sto}) on {side} side of the transformation"
                )
                continue
